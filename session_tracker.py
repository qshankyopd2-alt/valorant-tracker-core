...
def _summary(points: list[dict]) -> dict:
    ...
    return {
        "matches": len(rated), "totalMatches": len(points),
        "decisiveMatches": len(rated), "wins": wins, "losses": losses,
...
def _build_recap(lm, ingame_board: dict) -> dict | None:
    ...
    return {
        ...
        "resultExact": bool(detail.get("resultExact")),
...
def _point_from_recap(recap: dict) -> dict:
    ...
    return {
        ...
        "resultExact": bool(recap.get("resultExact")),
    }
    ...
    try:
        ...
        def finish():
            try:
                ...
                point = _point_from_recap(recap)
                competitive = (recap.get("mode") or "").lower() == "competitive"
                if competitive:
                    history.record(point, puuid=recap.get("puuid"),
                                   riot_id=recap.get("riotId"))
                with _LOCK:
