        ...
        with _LOCK:
            ...
            for match in matches:
                ...
                if not match.get("MatchID") or not ts:
                    continue
                # Competitive updates expose RR movement, not an authoritative
                # match outcome. A draw can still gain or lose RR, so keep the
                # result unresolved until match-detail enrichment supplies it.
                result = None
                point = {
    ...
    with _LOCK:
        ...
        candidates = [p for p in account.get("points", [])[-limit:]
                      if p.get("matchId") and (p.get("acs") is None
                                               ...
                                               or p.get("result") not in
                                               ("Victory", "Defeat", "Draw")
                                               or not p.get("resultExact"))]

    def fetch(point):
        try:
            live_match.ingest_match(
                point["matchId"], puuid,
                local_owner=puuid == live_match.self_puuid,
                provenance="local_history" if puuid == live_match.self_puuid else "profile_history",
            )
            return knowledge_store.career_match(point["matchId"], puuid)
        except Exception:
            return None
    ...
    if candidates:
        ...
        for row in rows:
            record({
                ...
                "resultExact": bool(row.get("resultExact")),
...
def _summary(points: list[dict]) -> dict:
    ...
    draws = sum(p.get("result") == "Draw" for p in points)
    unresolved = len(points) - len(rated) - draws
    ...
    return {
        "matches": wins + losses, "totalMatches": len(points),
        "decisiveMatches": wins + losses, "wins": wins, "losses": losses,
        "draws": draws, "unresolved": unresolved,
...
def _split_rows(points: list[dict], key: str) -> list[dict]:
    ...
    for name, matches in buckets.items():
        ...
        losses = sum(p.get("result") == "Defeat" for p in matches)
        draws = sum(p.get("result") == "Draw" for p in matches)
        unresolved = len(matches) - wins - losses - draws
        decided = wins + losses
        ...
        rows.append({
            "name": name, "games": len(matches), "decisiveMatches": decided,
    ...
    return sorted(
        ...
        key=lambda row: (
            -row["games"],
            -(row["winRate"] if row["winRate"] is not None else -1),
            row["name"],
        ),
    )
...
def _schedule_rows(points: list[dict], zone: ZoneInfo) -> dict:
    ...
    def summarize(name, matches):
        ...
        decided = wins + losses
        deltas = [p.get("delta") for p in matches if isinstance(p.get("delta"), (int, float))]
        return {"name": name, "games": len(matches), "decisiveMatches": decided,
                "wins": wins, "losses": losses, "draws": draws,
                "unresolved": unresolved,
                "winRate": round(100 * wins / decided) if decided else None,
                "netRr": sum(deltas)}
